...
export function describeAction(
  ...
  switch (name) {
    ...
    case 'create_dog':
      ...
      break
    case 'create_booking': {
      const dogIds = Array.isArray(args.dogIds) ? args.dogIds : []
      label = 'Create booking'
      description = `${dogIds.length} dog(s) · ${field('startAt')} → ${field('endAt')}`
      break
    }
    case 'set_booking_status':
    ...
    case 'create_invoice_from_booking':
      ...
      description = `Booking ${field('bookingId').slice(0, 8)}…`
      break
    case 'record_payment': {
      const cents = Number(args.amountCents ?? 0)
      label = 'Record payment'
      description = `$${(cents / 100).toFixed(2)} · ${field('method')}`
      break
    }
  }
